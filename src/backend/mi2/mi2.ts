...
export class MI2 extends EventEmitter implements IBackend {
  ...
  attach(
    ...
    return new Promise((resolve, reject) => {
      ...
      if (target.startsWith("extended-remote")) {
        ...
        if (executable)
          promises.push(
            this.sendCommand('file-symbol-file "' + escape(executable) + '"')
          );
      } else {
        ...
        if (executable)
          promises.push(
            this.sendCommand(
              'file-exec-and-symbols "' + escape(executable) + '"'
            )
          );
      ...
      promises.push(
        ...autorun.map((value) => {
          return this.sendUserInput(value);
        })
      );
      Promise.all(promises).then(() => {
        this.emit("debug-ready");
        ...
        parsed.outOfBandRecord.forEach((record) => {
          if (record.isStream) {
            ...
            if (record.type == "exec") {
              ...
              if (record.asyncClass == "running") this.emit("running", record);
  ...
  goto(filename: string, line: number, sessionId: number): Thenable<Boolean> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.sendCommand(
        "break-insert -t -f " + target + ` --session ${sessionId}`
      ).then(() => {
        this.sendCommand(
          "exec-jump " + target + ` --session ${sessionId}`
        ).then((info) => {
          resolve(info.resultRecords.resultClass == "running");
        }, reject);
      }, reject);
  ...
  async getStack(
    ...
    return stack.map((element: any) => {
      ...
      let is_boundary: boolean =
        MINode.valueOf(element, "boundary_frame") != undefined;
  ...
  async getStackVariables(
    ...
    const result = await this.sendCommand(
      `stack-list-variables --thread ${thread} --frame ${frame} --all-values`
    );
    ...
    if (!variables || !Array.isArray(variables)) {
      this.log(
        "stderr",
        `getStackVariables: variables is not iterable. Got: ${JSON.stringify(
          variables
    ...
    for (const element of variables) {
      ...
      if (!key || value === undefined) {
        // Skip if name is missing or value is not present
        continue;
      }
      ret.push({
        name: key,
  ...
  async varListChildren(
    ...
    const res = await this.sendCommand(
      `${miCommand} --all-values ${this.quote(name)}`
    );
    const children = res.result("children") || [];
    const omg: VariableObject[] = children.map(
      (child: any) => new VariableObject(child, threadId, parent)
    );
    return omg;
  }
