...
function parseString(str: string): string {
  ...
  for (let i = 0; i < str.length; i++) {
    if (escaped) {
      let m;
      if (str[i] == "\\") bufIndex += ret.write("\\", bufIndex);
      else if (str[i] == '"') bufIndex += ret.write('"', bufIndex);
      else if (str[i] == "'") bufIndex += ret.write("'", bufIndex);
      else if (str[i] == "n") bufIndex += ret.write("\n", bufIndex);
      else if (str[i] == "r") bufIndex += ret.write("\r", bufIndex);
      else if (str[i] == "t") bufIndex += ret.write("\t", bufIndex);
      else if (str[i] == "b") bufIndex += ret.write("\b", bufIndex);
      else if (str[i] == "f") bufIndex += ret.write("\f", bufIndex);
      else if (str[i] == "v") bufIndex += ret.write("\v", bufIndex);
      else if (str[i] == "0") bufIndex += ret.write("\0", bufIndex);
      else if ((m = octalMatch.exec(str.substring(i)))) {
        ret.writeUInt8(parseInt(m[0], 8), bufIndex++);
        i += 2;
      } else bufIndex += ret.write(str[i], bufIndex);
      escaped = false;
    } else {
      if (str[i] == "\\") escaped = true;
      else if (str[i] == '"') throw new Error("Not a valid string");
      else bufIndex += ret.write(str[i], bufIndex);
    }
  }
...
export class MINode implements MIInfo {
  ...
  record(path: string): any {
    if (!this.outOfBandRecord) return undefined;
    return MINode.valueOf(this.outOfBandRecord[0].output, path);
  }

  result(path: string): any {
    if (!this.resultRecords) return undefined;
...
export function parseMI(output: string): MINode {
  ...
  const parseCString = () => {
    ...
    while (inString) {
      ...
      else if (remaining[0] == '"') inString = false;
  ...
  const parseTupleOrList = () => {
    ...
    if (canBeValueList) {
      ...
      if (value !== undefined) {
        ...
        while ((value = parseCommaValue()) !== undefined) values.push(value);
        output = output.substring(1); // ]
        return values;
      }
    }
    ...
    if (result) {
      const results = [];
      results.push(result);
      while ((result = parseCommaResult())) results.push(result);
      output = output.substring(1); // }
      return results;
    }
  ...
  parseValue = () => {
    if (output[0] == '"') return parseCString();
    else if (output[0] == "{" || output[0] == "[") return parseTupleOrList();
    else return undefined;
  };
  ...
  while ((match = outOfBandRecordRegex.exec(output))) {
    ...
    if (match[2]) {
      ...
      if (!classMatch) {
        throw new Error("Failed to parse async class");
      }
  ...
  return new MINode(
    token ?? 0,
    outOfBandRecord || [],
    resultRecords ?? { resultClass: "", results: [] }
  );
}
