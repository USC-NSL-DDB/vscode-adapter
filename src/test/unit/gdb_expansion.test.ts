...
suite("GDB Value Expansion", () => {
  ...
  test("Various values", () => {
    ...
    assert.strictEqual(
      expandValue(variableCreate, `"hello world!"`),
      `"hello world!"`
    );
    assert.strictEqual(isExpandable(`0x7fffffffe956 "foobar"`), 0);
    assert.strictEqual(
      expandValue(variableCreate, `0x7fffffffe956 "foobar"`),
      `"foobar"`
    );
    ...
    assert.strictEqual(
      expandValue(variableCreate, `0x007ffff7ecb480`),
      "*0x007ffff7ecb480"
    );
    ...
    assert.strictEqual(
      isExpandable(`{[0] = 0x400730 "foo", [1] = 0x400735 "bar"}`),
      1
    );
    assert.deepStrictEqual(
      expandValue(
        variableCreate,
        `{[0] = 0x400730 "foo", [1] = 0x400735 "bar"}`
      ),
      [
        {
          name: "[0]",
  ...
  test("Error values", () => {
    ...
    assert.strictEqual(
      expandValue(variableCreate, `<No data fields>`),
      "<No data fields>"
    );
  });
  test("Nested values", () => {
    ...
    assert.deepStrictEqual(
      expandValue(variableCreate, `{a = {b = e}, c = d}`),
      [
        {
          name: "a",
          value: "Object",
  ...
  test("Simple node", () => {
    assert.strictEqual(
      isExpandable(`{a = false, b = 5, c = 0x0, d = "foobar"}`),
      1
    );
    const variables = expandValue(
      variableCreate,
      `{a = false, b = 5, c = 0x0, d = "foobar"}`
    );
